...
function ScoreRing({ score, color }: { score: number; color: string }) {
  ...
  const stroke = 6;
  const C = 2 * Math.PI * R;
  const pct = score / 12;  // max is now 12
  const dash = C * pct;
  const gap  = C - dash;
...
// ─── Score breakdown bar ──────────────────────────────────────────────────────
// Compact signed-contribution chips: "Delta +2" "Liquidity +0" etc.
// Reuses result.breakdown directly — no recalculation.

// Short display names for each criterion (keyed on the label prefix)
...
function shortLabel(full: string): string {
  for (const [prefix, short] of Object.entries(CRITERION_SHORT)) {
    if (full.startsWith(prefix)) return short;
  }
  // Fallback: first word
  return full.split(" ")[0];
}

function ScoreBreakdownBar({ breakdown }: { breakdown: ScoreBreakdownItem[] }) {
  return (
    <div>
      <p className="text-[10px] font-mono tracking-widest uppercase mb-2.5"
      ...
      <div className="flex flex-wrap gap-1.5">
        {breakdown.map((item, i) => {
          ...
          const color    = positive ? "#00e87a" : zero ? "#44445a" : "#ff3d6b";
          const bg       = positive ? "rgba(0,232,122,0.06)"
                         : zero     ? "rgba(68,68,90,0.12)"
                         : "rgba(255,61,107,0.06)";
          const border   = positive ? "rgba(0,232,122,0.18)"
                         : zero     ? "rgba(68,68,90,0.25)"
          ...
          return (
            <div
              key={i}
              title={item.detail}
              className="inline-flex items-center gap-1 rounded-md px-2 py-1 font-mono text-[10px]"
              style={{ color, background: bg, border: `1px solid ${border}` }}
            >
              <span style={{ color, opacity: 0.7 }}>{shortLabel(item.label)}</span>
              <span className="font-bold">
                {item.points > 0 ? "+" : ""}{item.points}
              </span>
              <span style={{ color, opacity: 0.4 }}>/{item.max}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative p-6 md:p-8 space-y-7">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <LiquidityBadge liquidity={result.liquidity} />
            </div>
          </div>
        </div>
        ...
        {/* ══ SCORE BREAKDOWN BAR ══ */}
        <ScoreBreakdownBar breakdown={result.breakdown} />
